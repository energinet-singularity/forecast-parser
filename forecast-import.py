...
def setup_ksql(kafka_topic: str, ksql_host: str, ksql_config: json):
    #Load setup of ksql streams and tables
    fields = ksql_config['fields']
    for config in ksql_config['config']:
    ...
    response = requests.post(f"http://{ksql_host}:8088/ksql",json={"ksql": f"LIST STREAMS; LIST TABLES;", "streamsProperties": {}})
    if response.status_code == 200:
        ...
        for ksql_item in ksql_config['config']:
            ...
            if ksql_item['NAME'] in ksql_existing_config[ksql_item['TYPE']]:
                ...
                response = requests.post(f"http://{ksql_host}:8088/ksql",json={"ksql": f"CREATE {ksql_item['TYPE']} {ksql_item['NAME']} {ksql_item['CONFIG']};", "streamsProperties": {}})
